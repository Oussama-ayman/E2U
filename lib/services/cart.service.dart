...
class CartServices {
  ...
  static Future<void> getCartItems() async {
    ...
    print("Cart service - Raw cart data: $cartList");
    ...
    if (cartList != null && cartList.isNotEmpty) {
      try {
        ...
        print(
          "Cart service - Successfully loaded ${productsInCart.length} items",
        );
      } catch (error) {
        print("Cart service - Error parsing cart data: $error");
        productsInCart = [];
      }
    } else {
      print("Cart service - No cart data found");
      productsInCart = [];
    }
  ...
  static addToCart(Cart cart) async {
    ...
    print("Adding to cart: ${cart.product?.name} (${cart.selectedQty})");
    ...
    try {
      ...
      await getCartItems();
      print(
        "Successfully added to cart. Total items: ${productsInCart.length}",
      );
    } catch (error) {
  ...
  static updateTotalCartItemCount(int total) async {
    ...
    print("Cart service - Updating stream with count: $total");
    cartItemsCountStream.add(total);
  }
