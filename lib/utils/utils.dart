...
class Utils {
  ...
  static Color textColorByBrightness([
    ...
    final ctx = context ?? AppService().currentContext ?? AppService().navigatorKey.currentContext;
    final isDark = ctx?.isDarkMode ?? false;
    if (reversed) {
  ...
  static Color greayColorByBrightness([BuildContext? context]) {
    final ctx = context ?? AppService().currentContext ?? AppService().navigatorKey.currentContext;
    final isDark = ctx?.isDarkMode ?? false;
    return isDark ? Colors.grey[300]! : Colors.grey[400]!;
  }

  static Color systemGreyColor([bool reversed = false]) {
    final ctx = AppService().currentContext ?? AppService().navigatorKey.currentContext;
    bool isDark = ctx?.isDarkMode ?? false;
    if (reversed) {
      isDark = !isDark;
    }
  ...
  static Color get primaryOrTheme {
    final ctx = AppService().currentContext ?? AppService().navigatorKey.currentContext;
    final isDark = ctx?.isDarkMode ?? false;
    final themeTextColor = ctx?.textTheme.bodyLarge?.color;
    return isDark ? (themeTextColor ?? Colors.white) : AppColor.primaryColor;
  }
