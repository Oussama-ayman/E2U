...
class LocationFetchViewModel extends MyBaseViewModel {
  ...
  Future<bool> locationPermissionGetter() async {
    bool granted = false;
    try {
      // First try the safer permission service
      final permissionService = AppPermissionHandlerService();
      granted = await permissionService.requestLocationPermissionSafely();

      // If that fails, fall back to the original logic
      if (!granted) {
      ...
      print("Error in locationPermissionGetter: $error");
      granted = false;
    }
