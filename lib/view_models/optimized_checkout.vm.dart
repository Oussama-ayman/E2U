...
class OptimizedCheckoutViewModel extends PaymentViewModel {
  //
  ...
  Future<void> placeOrderWithProgress() async {
    ...
    try {
      await optimizedProcessOrderPlacement().timeout(
  ...
  Future<void> optimizedProcessOrderPlacement() async {
    setBusy(true);
    ...
    try {
      ...
      // Pre-warm the network connection
      await _preWarmConnection();

      // Place order with optimized payload
      final orderPayload = _buildOptimizedOrderPayload();

      // Use parallel processing where possible
      final futures = <Future>[];

      // Start order placement
      final orderFuture = checkoutRequest
          .newOrder(checkout!, tip: driverTipTEC.text, note: noteTEC.text)
          .timeout(Duration(seconds: 30));

      futures.add(orderFuture);

      // Pre-clear cart in parallel (rollback if order fails)
      final clearCartFuture = CartServices.clearCart();
      futures.add(clearCartFuture);

      // Wait for order placement (most critical)
      final apiResponse = await orderFuture;

      if (apiResponse.allGood) {
        // Order successful, complete cart clearing
        await clearCartFuture;

        // Update wallet balance
        AppService().refreshWalletBalance.add(true);

        // Handle payment/navigation
        await _handleSuccessfulOrder(apiResponse);
      } else {
        // Order failed, restore cart if needed
        await _handleFailedOrder(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<void> _preWarmConnection() async {
    try {
      // Make a quick ping to the server to establish connection
      await checkoutRequest.get('/app/settings').timeout(Duration(seconds: 5));
    } catch (e) {
      print("Connection pre-warm failed: $e");
      // Don't block the process if pre-warm fails
    }
  }

  Map<String, dynamic> _buildOptimizedOrderPayload() {
    return {
      "tip": driverTipTEC.text,
      "note": noteTEC.text,
      "coupon_code": checkout?.coupon?.code ?? "",
  ...
  Future<void> _handleSuccessfulOrder(ApiResponse apiResponse) async {
    try {
      //cash payment
  ...
  Future<void> _handleFailedOrder(String? errorMessage) async {
    print("Order placement failed: $errorMessage");
    AlertService.error(
      context: viewContext,
      title: "Checkout".tr(),
      text: errorMessage ?? "Order placement failed",
    );
  }

  Future<void> _handleOrderError(dynamic error) async {
    print("Error during order placement: $error");
