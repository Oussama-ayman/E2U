...
class CartViewModel extends MyBaseViewModel {
  ...
  deleteCartItem(int index, {bool force = false}) async {
    print("Cart VM - Deleting item at index: $index, force: $force");
    //remove item/product from cart
    if (force) {
      cartItems.removeAt(index);
      print("Cart VM - Item removed, saving cart items: ${cartItems.length}");
    ...
    AlertService.confirm(
      ...
      onConfirm: () async {
        //remove item/product from cart
        print("Cart VM - User confirmed deletion, removing item at index: $index");
        cartItems.removeAt(index);
        print("Cart VM - Item removed, saving cart items: ${cartItems.length}");
        await CartServices.saveCartItems(cartItems);
        initialise();
  ...
  checkoutPressed() async {
    ...
    if (cartItems.isEmpty) {
      AlertService.error(
        title: "Empty Cart".tr(),
        text: "Please add items to your cart before checkout".tr(),
      );
      return;
    }
    ...
    print(
      "Cart items: ${cartItems.map((e) => '${e.product?.name} (${e.selectedQty})').toList()}",
    );
    ...
    print("Checkout created with ${checkOut.cartItems?.length} items");
    ...
    if (canOpenCheckout) {
      ...
      if (AppStrings.enableMultipleVendorOrder &&
        ...
        result = await Navigator.of(
          viewContext,
        ).pushNamed(AppRoutes.checkoutRoute, arguments: checkOut);
      }
  ...
  refreshState() async {
    ...
    await CartServices.updateTotalCartItemCount(count);
  }

  // Debug method to test cart functionality
  void debugCart() {
    print("=== CART DEBUG INFO ===");
    print("Cart items count: ${cartItems.length}");
    print("Cart services count: ${CartServices.productsInCart.length}");
    print("Total cart items: $totalCartItems");
    print("Sub total: $subTotalPrice");
    print("Discount: $discountCartPrice");
    print("Total: $totalCartPrice");

    for (int i = 0; i < cartItems.length; i++) {
      final item = cartItems[i];
      print(
        "Item $i: ${item.product?.name} - Qty: ${item.selectedQty} - Price: ${item.price}",
      );
    }
